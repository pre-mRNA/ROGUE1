...
def classify_polya_reads(df, output_dir, co_threshold=1.6, post_threshold=1.9):
    ...
    # ensure 'polya_length' is numeric and drop NaNs
    df['polya_length'] = pd.to_numeric(df['polya_length'], errors='coerce')
    df = df.dropna(subset=['polya_length'])

    # filter for alignment_length > 200
    # this removes short reads that are more promiscously polyadenylated 
    df = df[df['alignment_length'] > 200]

    # extract background and target data
    background_data = df[df['biotype'].isin(background_biotypes)]['polya_length'].values
    protein_coding_df = df[df['biotype'] == target_biotype].copy()
    ...
    # remove NaNs from background data
    background_data = background_data[~np.isnan(background_data)]
    if background_data.size == 0:
        logging.warning("All background reads have NaN polya_length. Skipping classification.")
        return df
