...
def run_bedtools(bam_file, genome_file, output_dir, dog_bed_file, exon_bed_file, intron_bed_file, gene_bed_file, num_files=104):
    ...
    with ThreadPoolExecutor(max_workers=num_files) as executor:
        ...
        for bed_chunk in temp_bed_files:
            intersect_exon_cmd = f"bedtools intersect -a {bed_chunk} -b {exon_bed_file} -wo -s -sorted -g {genome_file} | sort -k4,4 -k6,6 -k8,8nr --parallel=104 --buffer-size=32G > {bed_chunk}_exon_overlap.bed"
            intersect_intron_cmd = f"bedtools intersect -a {bed_chunk} -b {intron_bed_file} -wo -s -sorted -g {genome_file} | sort -k4,4 -k6,6 -k8,8nr --parallel=104 --buffer-size=32G > {bed_chunk}_intron_overlap.bed"
            if gene_bed_file:
                intersect_gene_cmd = f"bedtools intersect -a {bed_chunk} -b {gene_bed_file} -wo -s -sorted -g {genome_file} | sort -k4,4 -k6,6 -k8,8nr --parallel=104 --buffer-size=32G > {bed_chunk}_gene_overlap.bed"
            else:
                intersect_gene_cmd = f"touch {bed_chunk}_gene_overlap.bed"  # Create an empty file if we don't have a gene bed file
            dog_intersect_cmd = f"bedtools intersect -a {bed_chunk} -b {dog_bed_file} -wo -s -sorted -g {genome_file} | sort -k4,4 -k6,6 -k8,8nr --parallel=104 --buffer-size=32G > {bed_chunk}_dog_overlap.bed"
